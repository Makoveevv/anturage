...
### ⏸️ Припарковано: фича «Ваше фото» (заказ со своим изображением)
Заказчик попросил временно убрать. Код **не удалён**, а закомментирован с маркером `[parked:your-photo]`. Страницы `your-photo.html` нет (была бы 404), поэтому ссылки на неё отключены.
- **Баннер** «Заказ со своим изображением» — закомментирован в `src/index.html` (секция `.your-photo`).
- **Ссылки «Ваше фото»** в меню — закомментированы на всех 9 страницах в 3 местах (шапка `header__menu-bottom`, мобильное `navbar`, боковое `sidebar`).
- **Стили** — `src/assets/scss/home/_your-photo.scss` сохранён, но `@use` в `home.scss` закомментирован (чтобы мёртвый CSS не уезжал в прод).
- **Как вернуть**: снять обёртки `[parked:your-photo]` (поиск по проекту), создать страницу `your-photo.html`, раскомментировать `@use 'home/your-photo'` в `home.scss`, пересобрать (`gulp build`).
- Попутно исправлен баг: ссылка «Цены» в `navbar` на 4 страницах вела на `your-photo.html` → теперь на `price.html`.
